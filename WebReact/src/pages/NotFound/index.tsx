...
// modified code from https://codepen.io/code2rithik/pen/BapYBoP

const GLITCH_CHARS = '`¡™£¢∞§¶•ªº–≠åß∂ƒ©˙∆˚¬…æ≈ç√∫˜µ≤≥÷/?░▒▓<>/'.split('');

interface GlitchyTextProps extends HTMLAttributes<HTMLHeadingElement> {
    children: string;
}

interface CustomCSSProperties extends CSSProperties {
    [key: `--char-${number}`]: string; // Custom properties like --char-0, --char-1, etc.
    '--count'?: number; // Define '--count' as a number for clarity
}
...
const GlitchyText: React.FC<GlitchyTextProps> = ({ children, ...props }) => {
    return (
        <h1 {...props} className={`${styles['glitchy-text']} ${props.className || ''}`}>
            <span className={styles['glitchy-text__char--readable']}>{children}</span>
            {children.split('').map((char, idx) => {
                ...
                return (
                    <span
                        className={styles['glitchy-text__char']}
                        aria-hidden={true}
                        data-char={char}
                        key={`glitch-char--${idx}`}
                        style={charStyle}
...
const Logo = ({ className }: { className?: string }) => {
    return (
        <svg className={`${styles['bear-logo']} ${className}`} viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
            <g transform="matrix(1.34105 0 0 1.34105 -51.157 -1049.694)">
                ...
                />
                <path
                    className={styles['bear__tear-stream']}
                ...
                />
                <circle className={styles['bear__eye bear__eye--left']} r="11.185" cy="894.081" cx="97.242" />
                <circle className={styles['bear__eye bear__eye--right']} r="11.185" cx="201.851" cy="894.081" />
                <g className={styles['bear__shades']}>
...
function NotFound() {
    useEffect(() => {
        ...
        const update = (e: MouseEvent | DeviceMotionEvent) => {
            if ('acceleration' in e && e.acceleration?.x !== null) {
        ...
        <div className={styles.wrapper}>
            <main className={styles.appShell}>
                <div className={styles.container}>
                    <Link to="/" className={clsx(styles.message, styles['return-link'])}>
                        Go back to the homepage
                    </Link>
                    <Logo className={styles['bear-logo--tears']} />
                    <GlitchyText className={clsx(styles.code, styles['four-oh-four__code'])}>404</GlitchyText>
                    <GlitchyText className={clsx(styles.codeMessage, styles['four-oh-four__code-message'])}>
                        Not Found
                    </GlitchyText>
                </div>
            </main>
